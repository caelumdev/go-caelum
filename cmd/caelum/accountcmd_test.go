...
`)
	caelum.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(caelum.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(caelum.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	caelum.SetTemplateFunc("keypath", func(file string) string {
